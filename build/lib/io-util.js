...
module.exports = __toCommonJS(io_util_exports);
...
function newtonRaphson(f, x0, options) {
  ...
  while (iter++ < maxIter) {
    ...
    if (Math.abs(yp) <= epsilon * Math.abs(y)) {
      import_io_adapter.IoAdapter.logf.error("Newton-Raphson: failed to converged due to nearly zero first derivative");
      return false;
    }
    const x1 = Math.max(xMin, Math.min(xMax, x0 - y / yp));
    if (Math.abs(x1 - x0) <= tolerance * Math.abs(x1)) {
      if (verbose) {
        import_io_adapter.IoAdapter.logf.debug("Newton-Raphson: converged to x = " + String(x1) + " after " + String(iter) + " iterations");
      }
      return x1;
    }
    x0 = x1;
  }
  import_io_adapter.IoAdapter.logf.warn("Newton-Raphson: Maximum iterations reached (" + String(maxIter) + ")");
  return false;
}
class RLS {
  dimensions = 1;
  ...
  init(w, lambda, P) {
    ...
    import_io_adapter.IoAdapter.logf.debug("%-15s %-15s %-10s %s", this.constructor.name, "init()", "eye", JSON.stringify(this.eye));
    import_io_adapter.IoAdapter.logf.debug("%-15s %-15s %-10s %s", this.constructor.name, "init()", "w_hat", JSON.stringify(this.w_hat));
    if (typeof P === "number") {
      this.P = this.eye.multiply(P);
    } else if (P[0]) {
      this.P = import_numjs.default.array(P.flat()).reshape(P.length, P[0].length);
    }
    import_io_adapter.IoAdapter.logf.debug("%-15s %-15s %-10s %s", this.constructor.name, "init()", "P", JSON.stringify(this.P));
  }
